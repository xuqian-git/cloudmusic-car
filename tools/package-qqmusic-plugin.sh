...
OUT="$OUT_DIR/QQMusic-v1.0.13.ppmusic"
...
cd "$PROJECT_DIR"
./gradlew :qqmusic:clean :qqmusic:testReleaseUnitTest :qqmusic:bundleReleaseClassesToRuntimeJar
...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/qqmusic-plugin.XXXXXX")
...
cp "$PROJECT_DIR/qqmusic/src/main/res/drawable-nodpi/ic_qqmusic_app.png" "$TEMP_DIR/icon.png"
cp "$PROJECT_DIR/qqmusic/plugin.json" "$TEMP_DIR/plugin.json"
